...
const CartSection: React.FC  <CartProps>= ( { cartOpen, onClose, OrderPlaced,  CartData = [] } ) => {
                ...
                <div>
                        {CartData && CartData.map(( item:any, k:number ) =>{
                            ...
                            <div className="flex items-center space-x-4">
                              <div className="w-16 h-16 bg-green-400 flex items-center justify-center rounded-full"> <LazyLoadImage effect="blur" src= {  `${IMG_URL}/public/product/${item?.product_pics}`} alt={item?.title} className="w-16 h-16 object-contain" /> </div>
                            </div>

                              <div className="text-left md:max-w-[13rem] md:w-[13rem]  w-[10rem] md:max-w-[10rem]">
                                <p className="text-md  md:text-lg font-medium truncate ">{item?.name?.englishname}</p>
