...
const JustarrivedSection: React.FC = () => {
    ...
    <section className="py-10 overflow-hidden">
      <div className="max-w-1600 mx-auto px-4">
        <div className="md:flex justify-between items-center mb-6">
          <h2 className="text-2xl font-heading md:text-3xl font-semibold">{t("Just Arrived")}</h2>
          <div className="flex items-center gap-4  mt-[2rem] md:mt-0">
            <a href="#" className="text-blue-600 hover:underline text-sm font-medium"> {t("View All")} → </a>
          ...
          {products.map((product) => (
            <SwiperSlide key={product.id}>
                  <div className="relative p-4 bg-white border border-[#FBFBFB] shadow-[0px_5px_22px_rgba(0,0,0,0.04)] rounded-2xl mb-7 hover:shadow-[0px_21px_44px_rgba(0,0,0,0.08)] transition-shadow duration-300">
                      <button className="absolute top-5 right-5 w-[50px] h-[50px] rounded-full flex items-center justify-center bg-white border border-[#d8d8d8] hover:bg-red-500 hover:text-white transition-all duration-300"> <FaHeart />  </button>

                      <figure className="bg-[#F9F9F9] rounded-[12px] text-center mb-4">
                          <img src={product.image} alt="Product"  className="mx-auto max-h-[210px] h-auto" />
                      </figure>
